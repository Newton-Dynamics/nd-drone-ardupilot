...
local MODE_QHOVER     = 18           -- :contentReference[oaicite:1]{index=1}
...
local function clear_overrides()
    -- same pattern as your working script
    if rc and rc.clear_overrides then rc:clear_overrides() end  -- :contentReference[oaicite:2]{index=2}
end
...
local axis_ch     = nil
local axis_lbl    = ""
local baseline_pwm = nil   -- baseline stick value captured at start
...
function update()
    ...
    if running then
        ...
        if t_ms >= total_duration_ms() then
            clear_overrides()
            running = false
